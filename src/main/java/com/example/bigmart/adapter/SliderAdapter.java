...
public class SliderAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        ...
        bannercontainer.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor(sliderModelslist.get(position).getBackgroudncolor())));
        ImageView banner = view.findViewById(R.id.bannerslide);
        Glide.with(container.getContext()).load(sliderModelslist.get(position).getBanner()).apply(new RequestOptions().placeholder(R.drawable.ic_delete_black)).into(banner);
